...
function loadTris(n){
  for(let i = 0; i < n; i++){
    tris.push(new Ball(Math.random()*(canvas.width-2*this.radius)+this.radius, Math.random()*(canvas.height-2*this.radius)+this.radius, Math.random()*12-6, Math.random()*12-6, .3, Math.random()*30));
  }
}
